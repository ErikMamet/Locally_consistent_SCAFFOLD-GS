...
do
    ...
    do
        ...
        do
            # Loop over each entropy lambda (rate)
            for ((i=0; i<${#OFFSET_LAMBDA_LIST[@]}; i++));
            do
                OFFSET_LAMBDA=${OFFSET_LAMBDA_LIST[$i]}
                echo "Running $SCENE"
                # Set experiment name and output directory

                EXP_NAME=$RESULT_DIR/${SCENE}/$INIT_TYPE/GOP_$GOP_ID/r$RATE/off$OFFSET_LAMBDA  # <<<<<<<<<<<<<<<------------- EXP_NAME -------------<<<<<<<<<<<<<<<<<<<
